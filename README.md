...
Collection of Python scripts that I wrote for parsing/analysis of reduced representation sequencing data (e.g. RAD-seq, nextRAD). While many of the scripts are functional, some still need considerable cleaning up and more thorough testing - and this repository therefore very much represents a *work in progress*.

These scripts all require [Python 3](https://www.python.org/download/releases/3.0/), with some requiring additional packages ([BioPython](https://github.com/biopython/biopython.github.io/) and [NumPy](http://www.numpy.org/) - both of which can be easily installed using the [Miniconda](http://conda.pydata.org/miniconda.html) or [Anaconda](https://www.continuum.io/downloads) installers, or [PyVCF](https://github.com/jamescasbon/PyVCF) - which can be installed using e.g. `pip install PyVCF`). Usage information for each script can be obtained using the `-h` or `--help` flag (e.g. `python3 name_of_script.py -h` and is also listed in this README. Example input files are listed in this README as well.

This page is generated using the listed [README_compile.py](README_compile.py) script, and is based on the the [argparse](https://docs.python.org/3/library/argparse.html) information of each script.
...
vcf2outflank.py|Converts `.vcf` file to OutFLANK input f...|
vcf_ancestry_matrix.py|Creates a genotype matrix for loci that ...|
vcf_contrast_samples.py|Contrast all samples in `.vcf` file agai...|
vcf_find_clones.py|Script compares the allelic similarity o...|
vcf_gdmatrix.py|Calculates Genetic Distance (Hamming / p...|
vcf_genotype_freqs.py|Outputs genotype frequencies for specifi...|
vcf_get_chrom_pos_from_number.py|Translates sequential marker numbers bac...|
vcf_include_chrom.py|Retains only those loci (/CHROMs) in `.v...|
vcf_minrep_filter.py|Filters `.vcf` file for SNPs that are ge...|
vcf_missing_data.py|Outputs list of missing data (# and % of...|
vcf_pos_count.py|Counts SNP occurrence frequency for each...|
vcf_read_trim.py|Removes SNPs from `.vcf` that are above ...|
vcf_reference_loci.py|Lists all loci (using CHROM column) in `...|
vcf_remove_chrom.py|Excludes those loci (/CHROMs) in `.vcf` ...|
vcf_rename_loci.py|Renames CHROMS in `.vcf` file according ...|
vcf_rename_samples.py|Renames sample in `.vcf` file according ...|
