#!/usr/bin/env python
"""
Filters `.vcf` file for SNPs that are genotyped for a minimum proportion of
...
determine proportion of individuals genotyped (but all indivs are outputted).
"""
import sys
import vcf
import argparse
...
def prop_of_pop_genotyped(record, indvs):
...
def main(vcf_filename, pop_filename, threshold, output_filename):
    ...
    for record in vcf_reader:
        ...
        for pop in pops_indivs:
            if prop_of_pop_genotyped(record, pops_indivs[pop]) < threshold:
                failing_snp = True
                break
